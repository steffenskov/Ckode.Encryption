...
[assembly: InternalsVisibleTo("Ckode.Encryption.Tests")]
namespace Ckode.Encryption
{
	internal static class Extensions
	{
		...
		public static T[] GetSubArray<T>(this T[] array, int startIndex, int length)
		{
			...
			if (startIndex < 0)
			{
				throw new ArgumentOutOfRangeException(nameof(startIndex), $"startIndex ({startIndex}) cannot be less than zero.");
			}
			...
			{
				throw new ArgumentOutOfRangeException(nameof(length), $"startIndex ({startIndex}) plus length ({length}) cannot be higher than the length of the array ({array.Length}).");
			}
		...
		public static void Insert<T>(this T[] destinationArray, int startIndex, T[] subArray)
		{
			...
			{
				throw new ArgumentOutOfRangeException(nameof(subArray), $"Cannot insert subArray of length {subArray.Length} into destinationArray at position {startIndex}. Destination array isn't large enough (length: {destinationArray.Length}).");
			}
